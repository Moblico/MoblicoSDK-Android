...
public class MediaService {
    ...
    public static void findAllForLocation(final String category, final String mediaType, final String mediaTypeCategory, final Location location, final Callback<List<Media>> callback) {
        final Map<String, String> params = new HashMap<String, String>();
        if(category != null) params.put("category", category);
        if(mediaType != null) params.put("mediaType", mediaType);
        if(mediaTypeCategory != null) params.put("mediaTypeCategory", mediaTypeCategory);
        ...
        AuthenticationService.authenticate(new ErrorForwardingCallback<Void>(callback) {
            @Override
            public void onSuccess(Void result) {
                HttpRequest.get("locations/" + location.getId() + "/media", params, new ErrorForwardingCallback<String>(callback) {
